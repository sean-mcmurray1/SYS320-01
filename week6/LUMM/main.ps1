...
while($operation){
    ...
    elseif($choice -eq 3){ 
        ...
        if(-not (checkPassword $password)){
            Write-Host "The Password did not meet the requirements."
            return
        }
    ...
    elseif($choice -eq 7){
        ...
        $userLogins = (getLogInAndOffs $days | Where-Object { $_.User -ilike "$name"})
    ...
    elseif($choice -eq 8){
        ...
        $days = Read-Host "How many days back?"
        $userLogins = (getFailedLogins $days | Where-Object { $_.User -ilike "$name"})
        # TODO: Change the above line in a way that, the days 90 should be taken from the user
    ...
    elseif($choice -eq 9){
        ...
        Write-Host "Invalid selection. Please enter an integer 1-10."
    }
